...
def register_voice_routes(app):
    ...
    # ── 公开版语音通话（一次性密钥）──
    _used_keys = set()  # 已使用的密钥（内存中，重启清空）
    _active_public_sessions = set()  # 已验证的 session token

    @app.route('/voice/public')
    def voice_public_page():
        """公开版入口 — 显示密钥登录页或通话页"""
        from flask import request
        token = request.args.get('token', '')
        if token and token in _active_public_sessions:
            return render_template('panel_voice_public.html', token=token)
        return render_template('voice_login.html')

    @app.route('/api/voice/public/auth', methods=['POST'])
    def voice_public_auth():
        """验证一次性密钥"""
        ...
        if not key:
            return jsonify({'error': '请输入密钥'}), 400

        # 检查密钥
        valid_keys = _cfg('VOICE_PUBLIC_KEYS', [])
        if key not in valid_keys:
            return jsonify({'error': '密钥无效'}), 403

        if key in _used_keys:
            return jsonify({'error': '该密钥已使用'}), 403
        ...
        _active_public_sessions.add(token)
        logger.info(f"[VoiceCall] 公开版密钥验证成功: {key[:4]}***, token={token}")

        return jsonify({'token': token})

    @app.route('/api/voice/public/chat', methods=['POST'])
    def voice_public_chat():
        """公开版通话 — 验证 token 后走正常流程，不保存记录"""
        ...
        token = req.get('token', '')
        if token not in _active_public_sessions:
            return jsonify({'error': '未授权'}), 403

        user_text = req.get('text', '').strip()
        session_id = req.get('session_id', 'public_default')
        if not user_text:
            return jsonify({'error': '空消息'}), 400

        with _sessions_lock:
            if session_id not in _sessions:
                _sessions[session_id] = {'history': [], 'queue': _queue.Queue(), 'start_time': time.time(), 'public': True}
